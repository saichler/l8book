...
### l8events — Event Recording Service (required)
Required system service in every project, activated by l8common — projects never call `evtservices.ActivateEvents` themselves (no `main.go`/Dockerfile of its own). Single ORM-backed service `Events` (service area 76, `EventRecord`, PK `EventId`): POST generates ID/timestamps, PUT rejected (immutable), PATCH/GET allowed. Services post via `vnic.Resources().Events().PostXxxEvent(...)` (fire-and-forget) — `IEvents` is in l8types, the default impl and the 16 category-event-to-`EventRecord` converters are in `l8utils/go/utils/events`. Proto types live in `l8types/go/types/l8events`; UI components in `l8ui/events/`. Alarm lifecycle, archiving, and maintenance windows are **not** in l8events — use l8alarms.

### l8alarms — Full Alarm Management Application
Complete alarm lifecycle application (service area 10, prefix `/alm/`) with its own proto files, UI, mock data, and tests. Services: `Alarm`, `AlmDef` (definitions), `CorrRule` (correlation engine — topological/temporal/pattern/composite RCA), `NotifPol` (notification policies), `EscPolicy` (escalation scheduling), `AlmFilter`, `ArcAlarm` (archive), and `AlmOverlay` (read-only topology enrichment). It does **not** own events or notification delivery: alarms are created from shared `l8events.EventRecord`s pushed to the `Alarm` service over vnic (POST body is an `EventRecord`; no HTTP create — matched against `AlmDef` → DROP/CLEAR/MERGE/CREATE, then the source event is PATCHed to PROCESSED), and notification/escalation delivery goes through `Resources().Notify().Send` using the shared `l8notify.NotifyTarget`/`EscalationStep` types. Relies on the required l8events and l8notify services, which are activated by l8common (l8alarms does not activate them itself).

### l8notify — Multi-Channel Notification Delivery Service (required)
Required system service in every project, same shape as l8events (no `main.go`/Dockerfile of its own — activated by l8common, projects never activate it themselves). Two ORM-backed services in service area 78: `Notify` (`NotifyRecord` — immutable delivery log; `POST` performs the real dispatch in `Before()` and persists the outcome, `PUT` rejected) and `IntegCfg` (`IntegrationConfig` — fully editable integration endpoints: SMTP server, webhook, Slack, ...; non-secret fields only, `credential_key` looks up the secret in the project's security config `credentials` map). Any service sends through `vnic.Resources().Notify().Send(channel, endpoint, subject, body, attrs)` (interface in l8types, default impl in l8utils), which POSTs to `Notify` and returns the `DeliveryResult` synchronously. Proto types live in `l8types/go/types/l8notify`; UI components (`L8NotifyIntegrationMgmt`, delivery log, target editor) live in `l8ui/notify/`. The underlying `channel` (email/SMTP, HMAC-signed webhook, Slack, PagerDuty, custom senders), `template` (`{{key}}`), `throttle` (cooldown + hourly limit), and `escalation` (timed step chains) packages remain usable directly for bespoke policy flows that manage their own persistence.
...
2. **Identify needed services**: Every project uses l8types through l8common (the core stack). Every project also requires l8events and l8notify (system services activated by l8common — projects only register their types in the UI `main.go`). Most also need l8secure, l8logfusion, and l8ui. Observation projects additionally need l8pollaris, l8collector, l8parser, and l8inventory.

3. **Check for existing functionality** before building: l8services has FileStore, CsvExport, DataImport built in. l8events has event recording. l8alarms has full alarm management. l8notify has notification delivery as a service — send via `Resources().Notify().Send(...)`, never hand-roll SMTP/webhook/Slack code. l8topology has visualization. l8agent has AI chat.
